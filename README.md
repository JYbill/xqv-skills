...
用于为项目中的 Vitest 测试文件创建可提交、可跨团队使用的 WebStorm `.run/*.run.xml` 文件级运行配置。适合按测试目录或 glob 批量生成配置、使用 `folderName` 分组、参考已有 `.run` 模板，或为手动调试补齐 Vitest 配置。

使用原则：先读取仓库规则、测试脚本、Vitest 配置和已有运行模板，筛出真实测试文件，并用 `include`、`exclude` 等证据确认每个文件所属的 project、参数和环境变量；无法确认执行契约时停止生成，不能根据文件名或分组猜测。

补充约束：每个目标测试文件对应一份 `TEST_FILE` 配置，路径统一使用 `$PROJECT_DIR$`；同名时保留已有配置并添加最短唯一前缀。完成后检查文件映射、XML 和 diff，除非用户另有要求，不必执行测试本身。
