...
- `tsconfig.json`、`vite.config.ts`、`oxlint.config.ts`、`oxfmt.config.ts`；
- `lint-staged.config.js`、Oxlint、Oxfmt、Tailwind CSS、tsgo。

这份参考只覆盖 CSR React 项目的基础工具链，不包含业务打包模式、组件实现、路由、状态管理、接口封装、部署流程或测试框架。
...
2. **只保留基础工程链路。** 保留 React、Vite、Tailwind CSS、TypeScript、tsgo、Oxlint、Oxfmt、lint-staged、Husky 相关内容。
3. **按目标项目裁剪。** 如果目标项目不用 Tailwind CSS、Husky 或路径别名，移除对应依赖、插件、脚本和配置，不要留下空配置。
...
确认目标是 CSR React 项目
        │
        ▼
检查包管理器、React/Vite/Tailwind/TypeScript 现状
        │
        ▼
读取 references/react/package.md 和 dependencies.md
        │
        ▼
按目标项目替换包名、依赖版本和脚本
        │
        ▼
同步 tsconfig、vite、oxlint、oxfmt、lint-staged 配置
        │
        ▼
运行安装命令更新锁文件
        │
        ▼
运行 pnpm typecheck、pnpm lint、pnpm format --check 或项目等价命令
```
...
- `tsconfig.json`、`vite.config.ts` 和源码路径别名一致。
