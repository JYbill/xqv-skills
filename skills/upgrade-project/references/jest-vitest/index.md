...
10. 搜索 Jest 残留，例如 `jest`、`Jest`、`ts-jest`、`@types/jest`、`jest-e2e`、`.jest`、`jest.`。锁文件里的传递依赖名不一定要消失；非本次迁移的历史文档是否更新，按用户要求和项目约定判断。
...
- Jest 配置文件和 `test/jest-e2e.json` 已删除，或已确认不再被任何脚本引用。
