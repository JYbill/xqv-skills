...
新增模块和文件默认按真实依赖边界补同级 `*.spec.ts` 或 `*.integration-spec.ts`；涉及外部访问时补集成测试，纯本地逻辑补单元测试。只有明确指定的模块才补 `*.e2e-spec.ts`。
...
### 10. e2e 依赖指定的外部服务和测试数据

`test/**/*.e2e-spec.ts` 只用于明确指定的模块，并验证从输入到最终结果的完整业务链路。运行前确认影子 MySQL 数据库、Langfuse Dataset 等所需资源可用，不要用 mock 替代核心外部依赖。迁移现有 e2e 时只处理运行器切换和必要的资源关闭，不要扩大覆盖范围。
...
- `test/**/*.e2e-spec.ts` 仍只放在 `test/` 目录内，只覆盖明确指定的模块，并保留完整业务链路和真实外部依赖语义。
