...
- 仅用于真实模型调用，不纳入默认 test、test:watch、test:cov 或 e2e。
- 首次新增这类测试时，必须同步提供独立测试配置和执行命令；使用 Vitest 时创建 `vitest.llm.config.ts`，只收集 `src/**/*.llm-spec.ts`，并添加 `test:llm` 脚本：`vitest run --config vitest.llm.config.ts`。未完成入口配置前不新增该后缀文件。
- 仅在明确要求真实模型验证，并确认凭据、费用和专用测试数据后手动运行；不默认加入 CI。
- 设置超时，完成后关闭客户端并清理本次测试数据；断言结果结构与业务约束，避免要求生成文本逐字相同。
