...
1. 先按 `index.md` 确定目标稳定版本和仓库内版本声明，再使用该目标版本执行配置迁移。
...
4. 使用目标 pnpm 运行普通安装，受控更新 `pnpm-lock.yaml`。不要手工编辑 lockfile，也不要先删除 lockfile。
5. 执行以下最低验证：
...
6. 再运行项目已有的类型检查、构建和测试。存在 Docker / CI 调用方时，还要在对应环境输出 `pnpm --version` 并验证 frozen install，证明实际使用的是目标版本。

## 验收标准

- 本地、CI、Docker 和部署脚本没有相互矛盾的 pnpm 版本策略。
- 不再存在目标版本已移除的配置字段或 CLI 参数。
- `allowBuilds` 只包含经核对的依赖，严格构建检查没有被关闭。
- `pnpm-lock.yaml` 由目标版本受控更新，且 `pnpm install --frozen-lockfile` 通过。
