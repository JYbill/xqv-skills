...
默认更新 `AGENTS.md`，把项目提示词正文写入 `AGENTS.md`。`CLAUDE.md` 只保留下面一行：

```markdown
@AGENTS.md
```

除非用户或项目已有明确约定要求不同文件名，否则不要把完整约定正文写进 `CLAUDE.md`。

## `AGENTS.md` 基础模板

下面内容可作为后台项目的 `AGENTS.md` 基础模板。落地到具体项目时，按真实运行时、包管理器、目录结构、工具链和测试基础设施微调；不要把一次性迁移步骤或其他 reference 的配置片段写进项目提示词。
