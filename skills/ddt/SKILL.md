---
name: ddt
description: 检查并纠正代码里的过度封装、过度抽象、薄 helper、薄类型拆分、单次引用 enum / 常量、实例属性绕传参数和编码规范问题；当用户要求内联小函数、合并薄类型、内联单点常量、提升代码可读性或检查代码是否符合项目规范时使用。
---
...
| 某个 enum 或常量只有一个使用处                   | 是否应该直接耦合到使用处                |
...
如果一个 enum、`const`、`as const` 对象、延迟时间、阈值、固定 key、mode、status 或 tool name 只有一个使用处，默认这是过度抽取，应该直接耦合到使用处。`chatCopyButtonCopiedResetDelayMs = 100` 只有一个地方使用时，不应该抽成变量，而应该在使用处写 `100`。
...
| 只有一处引用的 enum、常量或 `as const` 对象                             | 多一次跳转，却没有共享或边界价值       |
| 只服务一次调用的 `chatCopyButtonCopiedResetDelayMs = 100`               | 变量名比使用处更长，还藏起了简单参数   |
...
只有一处引用的 enum、`const`、`as const` 对象、延迟时间、阈值、固定 key、mode、status、tool name，默认直接放在使用处。只有跨多个文件、多个分支、多个业务动作复用，或确实表达外部协议 / 复杂业务边界时，才抽到 `enum/**` 或常量文件。
...
下面这种常量抽取只有一个使用处，变量名没有提供新的业务边界，反而让读者多跳一次。
...
window.setTimeout(() => {
  setCopied(false);
}, chatCopyButtonCopiedResetDelayMs);
```

更合适的写法是直接把这个单次参数放回调用处。
