...
看到一个代码位置
        ...
        │        └─ 每个变量上方写 // 描述变量业务含义，for / if 上方写逻辑块目标或分支边界
        │
...
| 多变量准备后紧跟 for / if 逻辑处理       | 逐变量注释   | 每个变量说明业务含义，逻辑块说明过滤或分支边界 |
...
当同一逻辑阶段先连续定义多个变量，后面又紧跟 `for`、`if`、`switch` 等逻辑处理时，不能只用一条组注释覆盖整段。每个变量定义上方都要写 `// ...`，说明这个变量在后续逻辑里的业务含义或状态含义；每个 `for` / `if` 逻辑块上方都要写 `// ...`，说明该块负责的过滤、归一化、分支边界或产出结果。注释仍然不要解释语法本身，也不要写“定义变量”“遍历数组”这类表面描述。
